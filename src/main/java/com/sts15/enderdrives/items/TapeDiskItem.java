...
public class TapeDiskItem extends Item implements ICellWorkbenchItem, IMenuItem {
    ...
    public static void clearClientRequestCache() {
    ...
    public void addCellInformationToTooltip(ItemStack stack, List<Component> lines) {
        ...
        if (id != null) {
            ClientHooks.TapeInfo tapeInfo =
                    ClientHooks.getTapeInfo(id);

            int typeCount = tapeInfo.typeCount();
            long byteCount = tapeInfo.byteCount();
    ...
    public static UUID getTapeId(ItemStack stack) {
        ...
        try {
            return UUID.fromString(tag.getStringOr(TAPE_ID_KEY, ""));
    ...
    public static ItemStack deserializeItemStackFromBytes(byte[] data) {
        ...
        var provider = ServerLifecycleHooks.getCurrentServer().registryAccess();
        try {
            ItemStack result = StackCodecHelper.decodeItemStack(provider, tag);
        ...
        try {
            var legacyItemTag = tag.getCompound("tag");
            if (legacyItemTag.isPresent()) {
                CompoundTag tagTag = legacyItemTag.get();
            ...
            ItemStack fallback = StackCodecHelper.decodeItemStack(provider, tag);
