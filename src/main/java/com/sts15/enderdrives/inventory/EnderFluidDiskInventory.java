...
public class EnderFluidDiskInventory implements StorageCell {
    ...
    public EnderFluidDiskInventory(ItemStack stack) {
        ...
        this.disabled = item.isDisabled(stack) || !EnderFluidDiskItem.isScopeBound(stack);
    }

    @Override
    public CellState getStatus() {
        if (disabled || !running) return CellState.FULL;
    ...
    public double getIdleDrain() {
        if (disabled || !running) return 0.0;
    ...
    public long insert(AEKey what, long amount, Actionable mode, IActionSource source) {
        ...
        if (mode == Actionable.MODULATE) {
            accepted = EnderFluidDBManager.insertFluid(
                    scopePrefix, frequency, serialized, amount, typeLimit);
    ...
    public long extract(AEKey what, long amount, Actionable mode, IActionSource source) {
        if (disabled || amount <= 0 || !running) return 0;
        int transferMode = EnderFluidDiskItem.getTransferMode(stack);
        ...
        if (toExtract > 0 && mode == Actionable.MODULATE) {
            long persistedDelta = EnderFluidDBManager.saveFluid(
                    scopePrefix, frequency, serialized, -toExtract);
    ...
    public static CellState getCellStateForStack(ItemStack stack) {
        ...
        if (fluidDiskItem.isDisabled(stack) || !EnderFluidDiskItem.isScopeBound(stack) || !running) {
    ...
    public static byte[] serializeFluidStackToBytes(FluidStack stack) {
        try {
            ...
            CompoundTag tag = StackCodecHelper.encodeFluidStack(provider, stack);
    ...
    public static FluidStack deserializeFluidStackFromBytes(byte[] data) {
        try (DataInputStream dis = new DataInputStream(new ByteArrayInputStream(data))) {
            ...
            var provider = ServerLifecycleHooks.getCurrentServer().registryAccess();
            return StackCodecHelper.decodeFluidStack(provider, tag);
        } catch (Exception e) {
